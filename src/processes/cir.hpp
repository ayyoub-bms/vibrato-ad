...
public:
	...
	CIRProcess(State<T> &initialState, T kappa, T theta, T sigma) : Process<T>(initialState, "Cox-Ingeresoll-Ross"), speed(kappa), mean(theta), vol(sigma)
	{
	...
	{
		return Process<T>::describe(o) << "Kappa :" << speed << std::endl
									   << "Theta :" << mean << std::endl
									   << "Sigma :" << vol << std::endl;
	}
};
