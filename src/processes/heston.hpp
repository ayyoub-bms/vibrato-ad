...
class HestonProcess : public Process<T> {
	...
	public:
        ...
        T rate;
        T correlation;
		HestonProcess(State<T>& initialState, std::shared_ptr<CIRProcess<T>>& volProcess, T rate, T correlation) :
				Process<T>(initialState,  "Heston"), volProcess(volProcess), rate(rate), correlation(correlation){}
